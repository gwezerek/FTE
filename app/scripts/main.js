(function () {
  ...
  d3.json( 'data/2010_2011_nba.json', function(data) {
    ...
    g.selectAll( '.hexagon' )
      ...
      .enter().append( 'path' )
        ...
        // .attr( 'fill', function( d ) { return colorScale( d3.sum( d, function( shot ) { return shot.SHOT_MADE_FLAG; } ) / d.length ); } )
        .attr( 'fill', function( d ) { return colorScale( d.length ); } )
        .attr( 'transform', function( d ) { return 'translate( ' + d.x + ',' + d.y + ' )'; } );

    // let heatmap = document.querySelector(".clip-svg");
    // heatmap.style.display="none";
    // heatmap.offsetHeight;
    // heatmap.style.display="block";


    let heatmapData = convertForHeatmap( playerShots );
    simpleheat( 'canvas' )
      .data( heatmapData )
      .max( 20000 )
      .gradient( {
        0: 'rgb(215,48,39)',
        0.25: 'rgb(179,205,227)',
        0.5: 'rgb(140,150,198)',
        0.75: 'rgb(136,86,167)',
        1: 'rgb(129,15,124)'
      })
      // .gradient( {
      //   0: 'rgb(215,48,39)',
      //   0.125: 'rgb(244,109,67)',
      //   0.25: 'rgb(253,174,97)',
      //   0.375: 'rgb(254,224,144)',
      //   0.5: 'rgb(255,255,191)',
      //   0.625: 'rgb(224,243,248)',
      //   0.75: 'rgb(171,217,233)',
      //   0.875: 'rgb(116,173,209)',
      //   1: 'rgb(69,117,180)'
      // })
      .draw();
  ...
  function convertForHeatmap( playerShots ) {
    let hexData = hexbin( playerShots );
    let modHexData = [];

    // Ugh brute force
    _.each( hexData, function( bin, i ) {
      let binPercent = d3.sum( bin, function( shot ) { return shot.SHOT_MADE_FLAG; } ) / bin.length;
      for (var i = bin.length - 1; i >= 0; i--) {
        bin[i].binPercent = parseInt( binPercent * 1000 ) ;
        modHexData.push( bin[i] );
      };
    });

    let xCoords = _.map( modHexData, function( d ) { return xScale( d.LOC_X ); } );
    let yCoords = _.map( modHexData, function( d ) { return yScale( d.LOC_Y ); } );
    let made = _.map( modHexData, function( d ) { return 1000; } );
    // let made = _.pluck( modHexData, 'binPercent' );
    let heatmapData = _.zip( xCoords, yCoords, made );
  ...
  // simpleheat('canvas').data(data).max(18).draw();
